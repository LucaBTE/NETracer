...
/// Linux iputils backend. No terminal state or filesystem access.
...
async fn execute(mut command: Command, deadline: Duration) -> PingOutcome {
    let child = command.stdin(Stdio::null()).stdout(Stdio::piped())
        .stderr(Stdio::piped()).kill_on_drop(true).spawn();
    let mut child = match child {
        Ok(child) => child,
        Err(error) => return PingOutcome::Error(format!("Cannot start ping: {error}")),
    };
    ...
    // Drain both pipes while waiting; reap the process explicitly on timeout.
...
fn parse_output(code: Option<i32>, stdout: &[u8], stderr: &[u8]) -> PingOutcome {
    match code {
        Some(0) => {
            ...
            let latency_ms = text.lines()
                .find(|line| line.contains("min/avg/max"))
                .and_then(|line| line.split_once('='))
                .and_then(|(_, values)| values.trim().split('/').nth(1))
                .and_then(|value| value.parse::<f64>().ok())
                .filter(|value| value.is_finite() && *value >= 0.0);
            // A parsing failure does not turn a received reply into a lost packet.
            PingOutcome::Reply { latency_ms }
        }
        Some(1) => PingOutcome::NoReply,
        _ => {
            let message: String = String::from_utf8_lossy(stderr).chars()
                .filter(|character| !character.is_control()).take(240).collect();
