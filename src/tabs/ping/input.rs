...
pub(super) fn handle(tab: &mut PingTab, event: &Event) -> bool {
    match event {
        Event::Key(key) if key.kind != KeyEventKind::Release => {
            if tab.editing {
                match key.code {
                    KeyCode::Esc => tab.editing = false,
                    KeyCode::Enter if key.kind == KeyEventKind::Press => tab.submit(),
                    KeyCode::Enter => {}
                    ...
                    _ => { tab.input.handle_event(event); }
                }
                return true;
            }
            // Repeats are useful for navigation, but must not repeat destructive actions.
            match key.code {
                KeyCode::Up => tab.move_selection(-1),
                KeyCode::Down => tab.move_selection(1),
                KeyCode::Char('/') if key.kind == KeyEventKind::Press => tab.editing = true,
                KeyCode::Enter if key.kind == KeyEventKind::Press => tab.start_selected(),
                KeyCode::Delete if key.kind == KeyEventKind::Press => tab.delete_selected(),
                _ => return false,
            }
            true
        }
        Event::Paste(text) if tab.editing => {
            ...
            if text.len() > remaining || !text.is_ascii() || text.chars().any(char::is_control) {
                tab.message = "Paste a single ASCII hostname or IP address (max 254 characters).".into();
            } else {
                for character in text.chars() { tab.input.handle(InputRequest::InsertChar(character)); }
            }
            true
        }
        Event::Mouse(mouse) => {
            ...
            match mouse.kind {
                ...
                MouseEventKind::Down(MouseButton::Left) if tab.table_area.contains(position) => {
                    ...
                    if mouse.row >= first_row && mouse.row < tab.table_area.bottom().saturating_sub(1) {
