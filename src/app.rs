...
use crate::{components::Component, ui};
...
impl App {
    pub fn new(components: Vec<Box<dyn Component>>) -> io::Result<Self> {
        ...
        if components
            .iter()
            .any(|component| !ids.insert(component.id()))
        {
    ...
    pub fn select(&mut self, id: &str) -> io::Result<()> {
        let index = self
            .components
            .iter()
            .position(|component| component.id() == id)
            .ok_or_else(|| io::Error::other(format!("Unknown component: {id}")))?;
    ...
    fn handle_event(&mut self, event: &Event) {
        ...
        //Let focused components consume text before global shortcuts.
            ...
            && key.kind == KeyEventKind::Press
        {
            match key.code {
                ...
                KeyCode::Tab => {
                    self.activate((self.active + 1) % self.components.len());
                }

                KeyCode::BackTab => {
                    self.activate(
                        (self.active + self.components.len() - 1) % self.components.len(),
                    );
                }

                KeyCode::Char(digit @ '1'..='9') => {
                    self.activate((digit as u8 - b'1') as usize);
                }
